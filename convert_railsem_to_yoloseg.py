...
CLASS_MAP = {
    17: 0,  #rotaie
    18: 0,  #binari tram, li considero sempre come binari per una ragione di GENERALIZZAZIONE
    5 : 1,  #pali
    8 : 2,  #vegetazione
}
...
CLASSI_SOTTILI = {17,18}
CHIUSURA_PX = 6
DILATAZIONE_LEGGERA = 1
...
def mask_to_yolo_lines(mask: np.ndarray):
    ...
    for pixel_value, class_idx in CLASS_MAP.items():
        ...
        if pixel_value in CLASSI_SOTTILI:
            ...
            #Chiusura: collega i frammenti spezzati senza gonfiarli (no dilatazione)
            kernel_close = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (CHIUSURA_PX, CHIUSURA_PX))
            binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel_close)
            #Dilatazione leggera: per dare spessore minimo ai contorni
            kernel_dil = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (DILATAZIONE_LEGGERA, DILATAZIONE_LEGGERA))
            binary = cv2.dilate(binary, kernel_dil, iterations=1)
...
def main():
    ...
    #random.seed(42)
    #mask_files = random.sample(mask_files, 30)
    if not mask_files:
